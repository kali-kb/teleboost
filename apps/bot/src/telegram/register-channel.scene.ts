...
export class RegisterChannelWizard {
    ...
    async step1(@Ctx() ctx: Scenes.WizardContext) {
        ...
        try {
            const memberCount = await ctx.telegram.getChatMembersCount(state.channelId);
            ...
            await ctx.reply('I had trouble verifying your subscriber count automatically. Please ensure I am an admin in the channel and try again.');
            await ctx.scene.leave();
        }
    }
    ...
    @On('text')
    @WizardStep(2)
    async step2(@Ctx() ctx: Scenes.WizardContext, @Message('text') category: string) {
        const state = ctx.wizard.state as any;
    ...
    async step3(@Ctx() ctx: Scenes.WizardContext) {
        ...
        if (String(message.forward_from_chat.id) !== String(state.channelId)) {
            ...
            if (!state.wrong_channel_warned) {
                await ctx.reply('This post is from a different channel. Please forward posts specifically from your selected channel.');
        ...
        if (views !== undefined) {
            ...
            console.log(`Collected views for message ${message.message_id}: ${views}`);
        }
    ...
    private scheduleUIUpdate(ctx: Scenes.WizardContext) {
        ...
        this.uiTimers.set(chatId, setTimeout(() => {
            this.updateVerificationUI(ctx);
            this.uiTimers.delete(chatId);
        }, 1000)); // Wait 1 second for any other messages in the batch
    }
    ...
    private async updateVerificationUI(ctx: Scenes.WizardContext) {
        ...
        const avg = Math.round(state.view_counts.reduce((a: number, b: number) => a + b, 0) / count);
    ...
    async onFinishVerification(@Ctx() ctx: Scenes.WizardContext) {
        ...
        if (!state.view_counts || state.view_counts.length === 0) {
            await ctx.answerCbQuery('Please forward at least one post first!', { show_alert: true });
            return;
        }
    ...
    @On('text')
    @WizardStep(4)
    async step4(@Ctx() ctx: Scenes.WizardContext, @Message('text') country?: string) {
        const state = ctx.wizard.state as any;
    ...
    @On('text')
    @WizardStep(5)
    async step5(@Ctx() ctx: Scenes.WizardContext, @Message('text') city?: string) {
        const state = ctx.wizard.state as any;
    ...
    @On('text')
    @WizardStep(6)
    async step6(@Ctx() ctx: Scenes.WizardContext, @Message('text') language?: string) {
        const state = ctx.wizard.state as any;
    ...
    @On('text')
    @WizardStep(7)
    async step7(@Ctx() ctx: Scenes.WizardContext, @Message('text') niche?: string) {
        const state = ctx.wizard.state as any;
    ...
    @On('text')
    @WizardStep(8)
    async step8(@Ctx() ctx: Scenes.WizardContext, @Message('text') priceInput: string) {
        ...
        const { title, category, subscribers, avg_views, price_per_post, primary_country, primary_city, language, niche } = state;

        let details = `📢 **Channel:** ${title}\n` +
        ...
        await ctx.reply(`Please confirm the registration details:\n\n${details}\n` +
    ...
    async onConfirm(@Ctx() ctx: Scenes.WizardContext) {
        ...
        // const state = ctx.wizard.state as any;
        // await this.telegramService.saveChannel(state);

        await ctx.answerCbQuery();
        await ctx.reply('Channel registered successfully! 🎉 Welcome to Teleboost.');
        await ctx.scene.leave();
    }
