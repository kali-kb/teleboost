...
type AuthStep = 'signin' | 'signup' | 'profile-type' | 'company-details' | 'complete';
...
const passwordRequirements = computed(() => [
  ...
  { label: 'At least one symbol', met: /[!@#$%^&*(),.?":{}|<>]/.test(form.password) },
...
<template>
  <transition name="modal-fade">
    <div v-if="isOpen" class="fixed inset-0 z-[100] flex items-center justify-center px-4">
      <!-- Backdrop -->
      <div class="absolute inset-0 bg-slate-900/60 backdrop-blur-sm" @click="resetAndClose"></div>
      
      <!-- Modal Content -->
      <div class="relative w-full transition-all duration-500 overflow-hidden rounded-3xl bg-white shadow-2xl border border-slate-100"
        :class="{
          'max-w-2xl': currentStep === 'signup',
          'max-w-md': currentStep === 'signin' || currentStep === 'company-details',
          'max-w-lg': currentStep === 'profile-type' || currentStep === 'complete'
        }">
        ...
        <div class="p-6 md:p-8">
          ...
          <template v-if="currentStep === 'signin' || currentStep === 'signup'">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
              ...
              <div :class="[currentStep === 'signup' ? 'lg:col-span-5' : 'lg:col-span-12']">
                <!-- Google Auth -->
                <button class="w-full h-12 flex items-center justify-center gap-3 border border-slate-200 rounded-xl hover:bg-slate-50 transition-colors font-bold text-slate-700 mb-6 group">
                ...
                <div class="relative flex items-center mb-6">
                  ...
                  <span class="px-4 text-[10px] text-slate-400 font-black uppercase tracking-[0.2em]">Or Email</span>
                  <div class="flex-grow border-t border-slate-100"></div>
                </div>

                <div class="bg-slate-50 rounded-2xl p-5 border border-slate-100 hidden lg:block" v-if="currentStep === 'signup'">
                  <h4 class="text-xs font-black text-slate-700 uppercase tracking-widest mb-3">Strong Passwords</h4>
                  <div class="grid grid-cols-1 gap-2.5">
                    <div v-for="req in passwordRequirements" :key="req.label" 
                      ...
                      :class="req.met ? 'text-emerald-600 font-bold translate-x-1' : 'text-slate-400'">
              ...
              <div :class="[currentStep === 'signup' ? 'lg:col-span-7' : 'lg:col-span-12']">
                <form @submit.prevent="handleAuth" class="space-y-4">
                  <div v-if="currentStep === 'signup'" class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label class="block text-xs font-black text-slate-700 uppercase tracking-widest mb-1.5 ml-1">Full Name</label>
                      <input v-model="form.fullName" type="text" @input="validateField('fullName')"
                      ...
                      />
                      <p v-if="errors.fullName" class="text-rose-500 text-[10px] mt-1 font-bold uppercase tracking-tight">{{ errors.fullName }}</p>
                    </div>
                    <div>
                      <label class="block text-xs font-black text-slate-700 uppercase tracking-widest mb-1.5 ml-1">Email</label>
                      <input v-model="form.email" type="email" @input="validateField('email')"
                        class="w-full h-11 px-4 rounded-xl border border-slate-200 focus:border-primary focus:ring-4 focus:ring-emerald-50 outline-none transition-all placeholder:text-slate-300 bg-slate-50/30"
                        placeholder="business@example.et"
                      />
                      <p v-if="errors.email" class="text-rose-500 text-[10px] mt-1 font-bold uppercase tracking-tight">{{ errors.email }}</p>
                    </div>
                  </div>
                  ...
                  <div :class="[currentStep === 'signup' ? 'grid grid-cols-1 sm:grid-cols-2 gap-4' : '']">
                    <div>
                      <label class="block text-xs font-black text-slate-700 uppercase tracking-widest mb-1.5 ml-1">Password</label>
                      <div class="relative">
                        <input v-model="form.password" :type="showPassword ? 'text' : 'password'" @input="validateField('password')"
                          class="w-full h-11 px-4 rounded-xl border border-slate-200 focus:border-primary focus:ring-4 focus:ring-emerald-50 outline-none transition-all placeholder:text-slate-300 bg-slate-50/30"
                          placeholder="••••••••"
                        />
                    ...
                    <div v-if="currentStep === 'signup'">
                      <label class="block text-xs font-black text-slate-700 uppercase tracking-widest mb-1.5 ml-1">Confirm</label>
                      <input v-model="form.confirmPassword" type="password" @input="validateField('confirmPassword')"
                        class="w-full h-11 px-4 rounded-xl border border-slate-200 focus:border-primary focus:ring-4 focus:ring-emerald-50 outline-none transition-all placeholder:text-slate-300 bg-slate-50/30"
                        placeholder="••••••••"
                      />
                    </div>
                  ...
                  <div v-if="currentStep === 'signup' && form.password.length > 0" class="lg:hidden mt-3 grid grid-cols-1 gap-1.5 p-3.5 bg-slate-50 rounded-2xl border border-slate-100 shadow-inner">
                    <div v-for="req in passwordRequirements" :key="req.label" 
                      class="flex items-center gap-2.5 text-[11px] transition-all duration-300"
                      :class="req.met ? 'text-emerald-600 font-bold' : 'text-slate-400'">
                      <span class="material-symbols-outlined text-[16px] leading-none">
                        {{ req.met ? 'check_circle' : 'pending' }}
                      </span>
                      {{ req.label }}
                  ...
                  <p v-if="errors.password || errors.confirmPassword" class="text-rose-500 text-[10px] mt-1 font-bold uppercase tracking-tight">
          ...
          <template v-else-if="currentStep === 'profile-type'">
            <div class="flex justify-between items-start mb-6">
              <button @click="goBack" class="p-1.5 hover:bg-slate-100 rounded-xl text-slate-400 transition-all flex items-center gap-1 text-sm">
            ...
            <div class="text-center mb-8">
              <div class="inline-flex items-center justify-center size-16 bg-emerald-50 rounded-2xl mb-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <!-- Individual Option -->
              <button @click="selectAdvertiserType('INDIVIDUAL')" 
                ...
                <div class="size-12 bg-slate-100 group-hover:bg-emerald-100 rounded-xl flex items-center justify-center mb-4 transition-colors">
                  <span class="material-symbols-outlined text-2xl text-slate-600 group-hover:text-primary transition-colors">person</span>
                </div>
                <h3 class="font-bold text-slate-900 text-lg mb-1">Individual</h3>
                <p class="text-slate-500 text-sm leading-relaxed">For freelancers, creators, and personal brands running their own campaigns.</p>
              </button>

              <!-- Enterprise Option -->
              <button @click="selectAdvertiserType('ENTERPRISE')" 
                class="group p-6 rounded-2xl border-2 border-slate-200 hover:border-primary hover:bg-emerald-50/30 transition-all text-left">
                <div class="size-12 bg-slate-100 group-hover:bg-emerald-100 rounded-xl flex items-center justify-center mb-4 transition-colors">
                  <span class="material-symbols-outlined text-2xl text-slate-600 group-hover:text-primary transition-colors">domain</span>
                </div>
                <h3 class="font-bold text-slate-900 text-lg mb-1">Enterprise</h3>
                <p class="text-slate-500 text-sm leading-relaxed">For businesses and agencies managing multiple campaigns at scale.</p>
              </button>
            </div>
          </template>
          ...
          <template v-else-if="currentStep === 'company-details'">
            ...
            <div class="mb-6">
              <div class="inline-flex items-center justify-center size-14 bg-emerald-50 rounded-2xl mb-4">
              ...
              <p class="text-slate-500 mt-1 text-sm">Tell us about your business</p>
            </div>

            <form @submit.prevent="handleCompanySubmit" class="space-y-4">
              <div>
                <label class="block text-xs font-black text-slate-700 uppercase tracking-widest mb-1.5 ml-1">
                  Company Name <span class="text-rose-500">*</span>
                </label>
                <input v-model="companyForm.companyName" type="text" @input="validateField('companyName')"
                  class="w-full h-11 px-4 rounded-xl border border-slate-200 focus:border-primary focus:ring-4 focus:ring-emerald-50 outline-none transition-all placeholder:text-slate-300 bg-slate-50/30"
                  placeholder="Your Company Ltd."
                />
                <p v-if="errors.companyName" class="text-rose-500 text-[10px] mt-1 font-bold uppercase tracking-tight">{{ errors.companyName }}</p>
              </div>

              <div>
                <label class="block text-xs font-black text-slate-700 uppercase tracking-widest mb-1.5 ml-1">
                  Registration Number <span class="text-slate-400 font-medium normal-case tracking-normal">(Optional)</span>
                </label>
                <input v-model="companyForm.registrationNumber" type="text"
                  class="w-full h-11 px-4 rounded-xl border border-slate-200 focus:border-primary focus:ring-4 focus:ring-emerald-50 outline-none transition-all placeholder:text-slate-300 bg-slate-50/30"
              ...
              <div>
                <label class="block text-xs font-black text-slate-700 uppercase tracking-widest mb-1.5 ml-1">
                  Website <span class="text-slate-400 font-medium normal-case tracking-normal">(Optional)</span>
                </label>
          ...
          <template v-else-if="currentStep === 'complete'">
            <div class="text-center py-8">
              <div class="inline-flex items-center justify-center size-20 bg-emerald-100 rounded-full mb-6 animate-bounce">
                <span class="material-symbols-outlined text-4xl text-primary">celebration</span>
              </div>
              <h2 class="text-2xl font-black text-slate-900 tracking-tight mb-2">You're All Set!</h2>
              <p class="text-slate-500 text-sm max-w-sm mx-auto">
                Your {{ selectedAdvertiserType === 'ENTERPRISE' ? 'enterprise' : 'individual' }} advertiser account is ready. 
